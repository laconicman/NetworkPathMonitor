...
`NetworkObserver` answers one question well — *what is the network doing right now, and
...
Consumers depend on the ``NetworkPathMonitoring`` protocol rather than the concrete
``NetworkPathMonitor``, so the live monitor can be swapped for a scriptable stub in
